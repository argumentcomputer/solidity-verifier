...
library Step3Lib {
    ...
    function compute_claim_sat_final_r_sat_primary(
        ...
        PolyLib.SumcheckProof memory sumcheckProof = extract_sumcheck_proof(proof.sc_sat);
    ...
    function compute_c_secondary(
        ...
        // Question to reference implementation: Do we need this absorbing, that duplicates one above?
        transcript = KeccakTranscriptLib.absorb(transcript, label, evals);
    ...
    function compute_tau_primary(
        ...
        label[0] = 0x55;
        transcript = KeccakTranscriptLib.absorb(transcript, label, proof.r_U_primary);
